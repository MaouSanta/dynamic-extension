...
public:
  ...
  size_t offsetLinklist_{0},
      offsetLinklist0_{0}; // pre-blank to align the memory
  size_t offsetLinklistData_{0},
      offsetLinklistData0_{0}; // offset to the LinksData
  ...
  uint8_t *flash_data_dist_table_;

  HierarchicalNSWFlash(SpaceInterface<dist_t> *s) {}

  HierarchicalNSWFlash(SpaceInterface<dist_t> *s, const std::string &location,
  ...
  HierarchicalNSWFlash(SpaceInterface<dist_t> *s, size_t max_elements,
    ...
    size_links_level0_ = maxM0_ * sizeof(tableint) + maxM0_ * data_size_ +
                         sizeof(linklistsizeint);
    size_data_per_element_ =
        size_links_level0_ + data_size_ + sizeof(labeltype);
    offsetLinklist0_ = 0;
    offsetLinklistData0_ =
        maxM0_ * sizeof(tableint) + sizeof(linklistsizeint); // + tim;
  ...
  inline char *getLinksData(tableint internal_id, int level = 0) const {
    return level == 0
               ...
               : (linkLists_[internal_id] +
                  (level - 1) * size_links_per_element_ + offsetLinklistData_);
  }
#endif

  uint8_t *get_dist_table(tableint internal_id) {
  ...
  void set_dist_table(const void *data_point, tableint internal_id) {
    ...
    memcpy(flash_data_dist_table_ + subvector_num_ * cluster_num_ * internal_id,
           data_point, subvector_num_ * cluster_num_);
  }
  ...
  */
  inline void setLinksData(const void *data, size_t idx, tableint neighbor_id,
                           int level) const {
    ...
    uint8_t *currObj =
        ...
        sizeof(linklistsizeint) + block * VECTORS_PER_BLOCK * byte_num_;
  ...
  searchBaseLayer(tableint ep_id, const void *data_point, int layer) {
    ...
    while (!candidateSet.empty()) {
      ...
      dist_t *dist_list =
          (dist_t *)malloc((layer == 0 ? maxM0_ : maxM_) * sizeof(dist_t));
      PqLinkL2Sqr(dist_list, data_point, getLinksData(curNodeNum, layer), size,
                  layer);
#endif
...
// #endif
    ...
    for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
      ...
      if (!is_cur_c_present) {
        if (sz_link_list_other < Mcurmax) {
          ...
          PqLinkL2Sqr(dist_list, get_dist_table(selectedNeighbors[idx]),
          ...
          for (size_t j = 0; j < sz_link_list_other; j++) {
            ...
            candidates.emplace(
                ...
                data[j]);
#else
                fstdistfunc_(get_dist_table(data[j]),
                             getDataByInternalId(selectedNeighbors[idx]),
                             dist_func_param_),
                data[j]);
#endif
  ...
  //     uint8_t *data_ptr = (uint8_t *)data_ptrv;
  ...
  tableint addPoint(const void *data_point, labeltype label, int level) {
    ...
    memcpy(getDataByInternalId(cur_c),
           (uint8_t *)data_point + subvector_num_ * cluster_num_, data_size_);
      ...
      if (curlevel < maxlevelcopy) {
        ...
        for (int level = maxlevelcopy; level > curlevel; level--) {
          ...
          while (changed) {
            ...
            PqLinkL2Sqr(dist_list, data_point, getLinksData(currObj, level),
                        size, level);
#endif
            for (int i = 0; i < size; i++) {
              tableint cand = datal[i];
...
#if defined(PQLINK_CALC)
        dist_t *dist_list = (dist_t *)malloc(maxM_ * sizeof(dist_t));
        PqLinkL2Sqr(dist_list, query_data, getLinksData(currObj, level), size,
                    level, curdist);
#endif
        for (int i = 0; i < size; i++) {
          tableint cand = datal[i];
  ...
  /**
   * Calculate the squared Euclidean distance between a point and all its
   neighbors.
   * All calculations utilize
   [PSHUFB](https://www.felixcloutier.com/x86/pshufb),
   * where shuffling is constrained to 128-bit lanes with SSE, AVX/AVX2, or
   AVX512.

   * - The order of subvectors is rearranged for each data point.
   * - For AVX/AVX2 with 256-bit length, the first 4 subvectors are stored in
   the order [0, 2, 1, 3].
   * - Specifically, in one iteration, we load 256 bits from LinksData, which
   contains the encoded data for neighbor 0 and neighbor 1:
   *   - The first 128 bits contain subvector 0 data in the high (left) 4 bits
   of each byte and subvector 2 data in the low (right) 4 bits.
   *   - The second 128 bits contain subvector 1 data in the high (left) 4 bits
   of each byte and subvector 3 data in the low (right) 4 bits.
   * - The distance table is then shuffled:
   *   - The distance table for subvectors 0 and 1 is loaded into `qdists[0]`,
   while subvectors 2 and 3 are loaded into `qdists[1]`.
   *   - Shuffling is constrained within 128-bit lanes. The first 128 bits are
   shuffled with `qdists[0]`, and the second 128 bits with `qdists[1]`.
   * - A low_mask is used to extract the low 4 bits of each byte in the 128-bit
   lane, representing subvectors 2 and 3.
   * - `qdists[1]` directly provides the result for these subvectors.
   * - A right shift by 4 bits extracts the low 4 bits of each byte,
   representing subvectors 0 and 1.
   * - Finally, the two 128-bit lanes are summed to produce the final result.

   * @param result Pointer to a distance vector. Save the distance betwenn the
   points with all its neighbors in align.
   * @param pVect1v Pointer to a distance table. The distance table contains
   cluster_num_ distances for each subvector.
   * @param pVect2v Pointer to a distance table. The encoded data of all
   neighbors.
   * @param qty_ptr Pointer to the dimension of the vectors
   * @param level Level of the node since layer0 has 2M neighbors while others
   only have M neighbors
   */
  void PqLinkL2Sqr(const void *result, const void *pVect1v, const void *pVect2v,
                   size_t qty, size_t level = 0, dist_t dis = 0xff) const {
    ...
    const __m128i *qdists = reinterpret_cast<const __m128i *>(
        pVect1v); // qdists[i] for table of dim i
    const __m128i *part =
        reinterpret_cast<const __m128i *>(pVect2v); // part for iterating data

    size_t tmp = subvector_num_ / BATCH;
    for (int i = 0; i < BLOCKS; ++i) {
      if (qty <= VECTORS_PER_BLOCK * i)
        break;
      ...
      __m128i twolane_sum = _mm_add_epi8(
          ...
          _mm_shuffle_epi8(qdists[0],
                           _mm_and_si128(_mm_srli_epi64(comps, 4), low_mask)));
      part++;
      ...
      for (size_t j = 2; j < tmp; j += 2) {
        ...
        twolane_sum = _mm_add_epi8(
            ...
            _mm_add_epi8(_mm_shuffle_epi8(qdists[j | 1], masked),
                         _mm_shuffle_epi8(
                             qdists[j], _mm_and_si128(_mm_srli_epi64(comps, 4),
                                                      low_mask))));
        part++;
      }
    ...
    const __m256i *qdists = reinterpret_cast<const __m256i *>(
        pVect1v); // qdists[i] for table of dim i
                  // the distance table are aligned alloc by 32-byte
    const __m256i *part =
        reinterpret_cast<const __m256i *>(pVect2v); // part for iterating data
    ...
    for (size_t i = 0; i < BLOCKS; ++i) {
      ...
      for (size_t j = 0; j < tmp; j += 2) {
        ...
        twolane_sum = _mm256_add_epi8(
            twolane_sum,
            _mm256_add_epi8(
                _mm256_shuffle_epi8(qdists[j | 1],
                                    _mm256_and_si256(comps, low_mask)),
                _mm256_shuffle_epi8(
                    qdists[j],
                    _mm256_and_si256(_mm256_srli_epi64(comps, 4), low_mask))));
        ++part;
      }

      _mm_storeu_si128(reinterpret_cast<__m128i *>(res + i * VECTORS_PER_BLOCK),
                       _mm_adds_epi8(_mm256_castsi256_si128(twolane_sum),
                                     _mm256_extracti128_si256(twolane_sum, 1)));
    }
#else
  uint8_t *pVect1 = (uint8_t *)pVect1v; // 距离表
  uint8_t *pVect2 = (uint8_t *)pVect2v; // 所有的邻居编码
  memset(res, 0, qty * sizeof(dist_t));
  bool flag = true;
  uint8_t *pVect1_ls, *pVect1_rs;
  for (int i = 0; i < BLOCKS && flag; ++i) {
    ...
    for (int j = 0; j < byte_num_; ++j) {
      for (int k = 0; k < VECTORS_PER_BLOCK; ++k) {
        ...
        res[i * VECTORS_PER_BLOCK + k] += (dist_t)pVect1_ls[ls(*pVect2)];
        res[i * VECTORS_PER_BLOCK + k] += (dist_t)pVect1_rs[rs(*pVect2)];
        pVect2++;
      }
      pVect1_ls += cluster_num_ << 1;
      pVect1_rs += cluster_num_ << 1;
    }
  }
#endif
  }
