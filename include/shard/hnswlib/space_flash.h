 ...
 * CLUSTER_NUM distances for each subvector.
 ...
 */
static float FlashL2Sqr(const void *pVect1v, const void *pVect2v,
                        const void *qty_ptr) {
  uint8_t *pVect1 = (uint8_t *)pVect1v; // distance table
  uint8_t *pVect2 = (uint8_t *)pVect2v; // encoded data
  size_t qty = *((size_t *)qty_ptr);

  float res = 0;
  for (int i = 0; i < qty; ++i) {
    res += (float)pVect1[ls(pVect2[i])];
    pVect1 += 16;
    res += (float)pVect1[rs(pVect2[i])];
    pVect1 += 16;
  }
  return (res);
}

class FlashSpace : public SpaceInterface<float> {
